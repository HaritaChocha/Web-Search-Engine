 ...
 * values cannot be <tt>null</tt>&mdash;setting the value associated with a key
 * to <tt>null</tt> is equivalent to deleting the key from the symbol table.
 * <p>
...
public class TrieST<Value> {
	private static final int R = 25600; // extended ASCII
	...
	public static List<String> stopwords = Arrays.asList("a", "able", "about", "across", "after", "all", "almost",
	...
	public void put(String key, Value val) {
		...
		// System.out.println(key + " " + root.count);
	}
	...
	// int wc = 0;
	// wc = getCount(root, key, 0);
	// return wc;
	// }
	//
	// private int getCount(Node x, String key, int d) {
	// if (d == key.length()) {
	// return x.count;
	// }
	// char c = key.charAt(d);
	// return getCount(x.next[c], key, d+1);
	// }
	 ...
	 * @param pattern
	 *            the pattern
	 * @return all of the keys in the symbol table that match <tt>pattern</tt>, as
	 *         an iterable, where . is treated as a wildcard character.
	 */
	...
	public static void createTrie(String file, TreeMap<String, TreeMap<Integer, Integer>> trieMap, int DocID) {
		int size = 0;
		String[] keys = file.split(",|\\s|:|\\.|\\?|\\(|\\)|\\{|\\}|\\[|\\]|-|<|>|=|;|\\/|[0-9]|'|\\{");
		TrieST<Integer> st = new TrieST<Integer>();
		for (int i = 0; i < keys.length; i++) {
			st.put(keys[i].toLowerCase(), i);
		}
		for (String key : st.keys()) {
			if (st.get(key).size() > 0 && !stopwords.contains(key)) {
				trieMap.put(key, new TreeMap<Integer, Integer>());
				trieMap.get(key).put(DocID, st.get(key).size());
				size += st.get(key).size();
			}
		}
		wordCount = size;
		
		// Create Inverted Index with the trie
		InvertedIndex.createInvertedIndex(trieMap, size, DocID);
	}
